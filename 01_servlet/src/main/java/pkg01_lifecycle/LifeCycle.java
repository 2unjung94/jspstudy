 ...
 *    ( = 패키지가 달라도 한번 프로젝트에서 썻던 이름은 두번다시 쓸 수 없다.)
 ...
 * 
 * 1. 형식
 *    protocol://host:portcontextPath/URLMapping (contextPath 는 /부터 시작한다)
 * 2. contextPath
 *    1) 프로젝트 경로를 의미한다
 *    2) 프로젝트를 생성할 때 결정한다.
 *    3) 프로젝트 속성(Properties)에서 변경할 수 있다.
 *      프로젝트 우클릭 - 속성(Properties) - Web Project Settings
 * 3. URLMapping
 *    1) 프로젝트 내부 경로를 의미한다.
 *    2) Servlet을 생성할 때 결정한다.
 *    3) 변경 방법
 *      (1) @WebServlet Annotation (기본적으로 import 해야하지만 현재 10버전은 Servlet 생성시 import 되어있다)
 *      (2) WEB-INF/web.xml
 *      
 */
...
public class LifeCycle extends HttpServlet {
  ...
  public LifeCycle() {
	 ...
	 * 
	 * init() 메소드
	 * 1. Servlet 환경 설정을 담당하는 메소드이다. (환경 설정 할거 아니면 추가할 필요 없다)
	 ...
	 * 
	 * service() 메소드 (doGet, doPost가 있으므로 필요 없다)
