...
<%--
  ...
  2. Bind 영역에 저장된 값을 나타낼 수 있다. (기억해둘것)
...
<%--
  ...
  2. request     : HttpServletRequest : 응답 전까지 접근 가능하다.(일회용 저장소)
...
<body>
  ...
  <%-- Java 에서 Getter, Setter 꼭 만들어놓고 사용해야 한다 --%>
  <div>${book.title}</div>  <%-- book.getTitle() 방식으로 호출 된다 --%>
  <div>${book.author}</div> <%-- book.getAuthor() 방식으로 호출 된다 --%>
  <div>${book.price}</div>  <%-- book.getPrice() 방식으로 호출 된다 --%>
  <hr>
  ...
  <%
    List<Book> books = Arrays.asList(new Book("태백산맥", "조정래", 1000),
                                    new Book("홍길동전", "허균", 2000),
                                    new Book("데미안", "헤르만헤세", 1400));
  ...
  <div>${a - b}</div>
  <div>${a * b}</div>
  <div>${a / b}, ${a div b}</div> <%-- div: EL 전용 연산자 --%>
  <div>${a % b}, ${a mod b}</div> <%-- mod: EL 전용 연산자 --%>
  <hr>
  ...
  <%--
    ...
    1. 속성(Attribute)
      ...
      ${number}, 숫자 타입 ( 10 > 5 : true 를 10 - 5 > 0 이렇게 크기 비교 한다.)
    
    2. 파라미터(Parameter)
      response.sendRedirect("/contextPath/URLMapping?number=10");
    
      ${param.number}, 문자열 타입 ( "10" > 5 : false 를 "10" - 5 > 0 이렇게 크기 비교 한다. )
  --%>
